...
export default function Home() {
	...
	const handleEnterprise = () => {
		const email = "hardikatbennett@gmail.com";
	...
	const handleGithub = () => {
		window.open("https://github.com/HKgame01/");
	};
	const handleTwitter = () => {
		window.open("https://x.com/hkag01");
	};
	...
	const Testimonials = () => {
		...
		const testimonials = [
			"Innota has revolutionized how I manage my research. The ability to chat with my collected knowledge is game-changing. It's like having a personal research assistant available 24/7.",
			"With Innota, I no longer waste hours sifting through PDFs and videos. The retrieval tool surfaces exactly what I need in seconds.",
			"Keeping track of internal documents and resources used to be a nightmare. Innota is the ultimate knowledge concierge for my team.",
			"As a video creator, organizing and referencing my material used to be a headache. Innota’s ability to parse video transcripts is a game-changer.",
			"I deal with massive datasets and complex documentation. Innota ensures I never lose track of crucial insights. It's like having an AI-powered memory.",
		];
		useEffect(() => {
			fetch("https://randomuser.me/api/?results=3")
				.then((response) => response.json())
				.then((data) => {
					const formattedUsers = data.results.map((user: any, idx: number) => ({
			...
			<div className="p-8 rounded-xl bg-neutral-800">
				...
				<div className="flex gap-1">
					{Array(5)
						.fill(null)
						.map((_, idx) => (
							<Star className="fill-indigo-500 stroke-none" key={idx} />
						))}
				</div>
			</div>
		);
			...
			<div>
				<div className="text-center">
					...
					<div className="max-w-3xl mx-auto px-4 flex flex-col gap-6 py-16">
						{faqs.map((faq, i) => (
							<FaqButton question={faq.question} answer={faq.answer} key={i} />
						))}
					</div>
				</div>
			</div>
			...
			<div className="max-w-7xl mx-auto  px-4 md:px-8 pt-20 pb-10">
				<div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16   ">
					<div className="col-span-1 md:col-span-2">
						<h1 className="text-white font-bold text-2xl mb-6">Innota</h1>
				...
				<div className="border-t border-neutral-800 pt-8">
					<div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0">
						<div className="text-gray-400 text-sm">
							© 2024 Innota. All rights reserved.
						</div>
