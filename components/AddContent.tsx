...
export default function AddContent({
	...
	const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || ""; // ✅ Next.js env
	...
	useEffect(() => {
		const getStatus = async () => {
			...
			const contentStatusData = await getContentUploadStatus();
				...
				return (
					<div className="mt-4 rounded-md border border-purple-100 dark:bg-stone-900 dark:border-stone-700 p-4 shadow-md">
						<h1 className="text-sm font-semibold dark:text-gray-300">
							We are working on X and Notion integrations
						</h1>
					</div>
				);
		}
	...
	const handleAddMemory = async (e: FormEvent<HTMLFormElement>) => {
		try {
				...
				},
				{
					...
					error: (err) => {
						...
						return statusCode === 429
							...
							: `Failed to create memory (status: ${statusCode})`;
					},
					duration: 3000,
				}
			);
			...
			toast.error("Unable to create memory", { duration: 3000 });
		}
	};
